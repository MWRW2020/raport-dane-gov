...
def create_dataset_structure(root):
    ...
    # Tworzenie elementu <dataset> z przestrzenią nazw
    # Status "draft" na czas testów. Docelowo: 'published' [3, 4].
    dataset = ET.SubElement(root, f"{{{CONFIG['SCHEMA_URL']}}}dataset", status='draft') 
    
    ET.SubElement(dataset, 'extIdent').text = f"dataset_{CONFIG['INWESTYCJA_ID']}" [5]
    
    title_dataset = ET.SubElement(dataset, 'title') [5]
    ET.SubElement(title_dataset, 'polish').text = f"Ceny ofertowe mieszkań dewelopera {CONFIG['DEWELOPER_NAZWA']} w {CONFIG['ROK_ZBIORU']} r." [5]
    ET.SubElement(title_dataset, 'english').text = f"Offer prices of apartments of developer {CONFIG['DEWELOPER_NAZWA']} in {CONFIG['ROK_ZBIORU']}." [5]
    
    description_dataset = ET.SubElement(dataset, 'description') [5]
    desc_text_pl = (f"Zbiór danych zawiera informacje o cenach ofertowych mieszkań dewelopera {CONFIG['DEWELOPER_NAZWA']} "
                    ...
                    f"mieszkalnego lub domu jednorodzinnego oraz Deweloperskim Funduszu Gwarancyjnym (Dz. U. z 2024 r. poz. 695).") [6, 7]
    ET.SubElement(description_dataset, 'polish').text = desc_text_pl [5]
    ET.SubElement(description_dataset, 'english').text = desc_text_pl [5]
    
    ET.SubElement(dataset, 'url').text = CONFIG['INWESTYCJA_URL'] [2]
    
    # *******************************************************************
    # PRAWIDŁOWA KOREKTA BŁĘDU: Wymagana zagnieżdżona struktura dla "ECON" [1, 2].
    categories = ET.SubElement(dataset, 'categories')
    ET.SubElement(categories, 'category').text = 'ECON'
    # *******************************************************************
    
    tags = ET.SubElement(dataset, 'tags') [2]
    ET.SubElement(tags, 'tag').text = 'deweloper'
    
    ET.SubElement(dataset, 'updateFrequency').text = 'daily' [1, 2]
    ET.SubElement(dataset, 'hasDynamicData').text = 'false' [1, 8]
    ET.SubElement(dataset, 'hasHighValueData').text = 'true' [1, 8]
    ET.SubElement(dataset, 'hasHighValueDataFromEuropeanCommissionList').text = 'false' [1, 8]
    ET.SubElement(dataset, 'hasResearchData').text = 'false' [1, 8]
...
def create_resource_element(resources_element_et, today_str, today_str_compact, daily_data_url):
    ...
    # Docelowo status powinien być 'published' [4].
    resource = ET.SubElement(resources_element_et, 'resource', status='draft')
    
    ET.SubElement(resource, 'extIdent').text = ext_ident_today [9]
    ET.SubElement(resource, 'url').text = daily_data_url [9]
    
    title_resource = ET.SubElement(resource, 'title') [9]
    ET.SubElement(title_resource, 'polish').text = f"Ceny ofertowe mieszkań dewelopera {CONFIG['DEWELOPER_NAZWA']} {today_str}" [10]
    ET.SubElement(title_resource, 'english').text = f"Offer prices for developer's apartments {CONFIG['DEWELOPER_NAZWA']} {today_str}" [10]
    
    description_resource = ET.SubElement(resource, 'description') [10]
    ...
    ET.SubElement(description_resource, 'polish').text = desc_res_text_pl [10]
    ET.SubElement(description_resource, 'english').text = desc_res_text_pl [10]
    
    ET.SubElement(resource, 'availability').text = 'local' [11]
    ET.SubElement(resource, 'dataDate').text = today_str [11, 12]

    special_signs = ET.SubElement(resource, 'specialSigns') [12]
    ET.SubElement(special_signs, 'specialSign').text = 'X' [12]
    
    ET.SubElement(resource, 'hasDynamicData').text = 'false' [13]
    ET.SubElement(resource, 'hasHighValueData').text = 'true' [13]
    ET.SubElement(resource, 'hasHighValueDataFromEuropeanCommissionList').text = 'false' [13, 14]
    ET.SubElement(resource, 'hasResearchData').text = 'false' [14]
    ET.SubElement(resource, 'containsProtectedData').text = 'false' [14]
...
def format_xml(root_element):
    ...
    """
    # 1. Serializacja do stringu z utf-8 (z przestrzeniami nazw)
    xml_string_raw = ET.tostring(root_element, 'utf-8', method='xml')
    
    # 2. Używamy minidom do formatowania (pretty-print)
    parsed_string = minidom.parseString(xml_string_raw)
    
    # toprettyxml zwraca string w kodowaniu utf-8
    pretty_xml = parsed_string.toprettyxml(indent="  ", encoding="utf-8")
    
    # 3. Usuwanie niepotrzebnych deklaracji XML i białych znaków
    pretty_xml_str = pretty_xml.decode('utf-8')
    lines = pretty_xml_str.split('\n')
    
    # Filtrowanie i przywracanie prawidłowej deklaracji root dla przestrzeni nazw
    formatted_lines = [line for line in lines if line.strip() and not line.startswith('<?xml')]
    
    # Przywrócenie przestrzeni nazw w elemencie root (ElementTree gubi ją w parsowaniu)
...
def generate_xml_and_md5():
    ...
    if root_et is None:
        ...
        # Tworzenie elementu głównego: p:datasets (tylko jako placeholder)
    ...
    # Suma kontrolna musi być liczona z finalnej zawartości XML [15, 16].
    md5_sum = hashlib.md5(final_xml_data).hexdigest()
    md5_file_path = f"{CONFIG['XML_FILENAME']}.md5"
    
    # Hash musi być małymi literami [16].
